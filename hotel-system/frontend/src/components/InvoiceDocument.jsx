...
export default function InvoiceDocument({ reservationId }) {
  ...
  if (error || !data) {
    return <div className="text-center py-32 text-red-400">{error || t("noData")}</div>;
  }
  ...
  /* Rate fallback chain: room → snapshot → derived → dash */
  const roomRate = room?.basePrice ?? ps.basePrice ?? (nights > 0 ? Math.round((ps.roomCharge || 0) / nights) : null);
  const extraBedQty = (reservation.extraBeds || 1) * nights;
  const extraBedRate = room?.extraBedPrice ?? ps.extraBedPrice ?? (extraBedQty > 0 ? Math.round((ps.extraBedCharge || 0) / extraBedQty) : null);
  const overtimeRate = room?.overtimeHourlyRate ?? ps.overtimeHourlyRate ?? null;
  const rateOrDash = (rate) => (rate == null ? "-" : formatMoney(rate));

  const methodLabels = { cash: t("cash"), card: t("card"), bank_transfer: t("bankTransfer"), other: t("other") };
      ...
      <div className="invoice-header bg-gradient-to-r from-purple-900 to-indigo-900 text-white px-8 py-5">
        <div className="flex justify-between items-start gap-6">
          <div className="flex-1">
            {settings?.logoUrl && (
              <img src={`${window.location.origin.startsWith("http") ? "" : ""}${settings.logoUrl}`} alt="" style={{ display: "none" }} />
            )}
        ...
        <div className="grid grid-cols-2 gap-5 mb-6">
          <div className="print-box border border-gray-300 rounded-md p-4">
            ...
            <div className="mt-1.5 text-xs text-gray-600 space-y-1">
              ...
              <p><strong>{guest.guestType === "foreigner" ? "Passport" : "NRC"}:</strong> {guest.guestType === "foreigner" ? guest.passport : guest.nrc}</p>
        ...
        <table className="w-full border-collapse mb-6">
          ...
          <tbody className="text-xs">
            ...
            {(ps.extraBedCharge || 0) > 0 && (
              <tr className="border-b border-gray-300">
                <td className="py-2.5 px-3"><p className="font-medium text-gray-900">Extra Bed</p><p className="text-[10px] text-gray-500">Additional bed charge</p></td>
            ...
            {(ps.overtimeCharge || 0) > 0 && (
              <tr className="border-b border-gray-300">
                <td className="py-2.5 px-3"><p className="font-medium text-gray-900">Overtime Charges</p><p className="text-[10px] text-gray-500">Late check-out fee</p></td>
                <td className="py-2.5 px-3 text-center">-</td>
            ...
            {(ps.voucherDiscount || 0) > 0 && (
              <tr className="border-b border-gray-300">
                <td className="py-2.5 px-3"><p className="font-medium text-gray-900">Voucher Discount</p>{reservation.voucherCode && <p className="text-[10px] text-gray-500">Code: {reservation.voucherCode}</p>}</td>
                <td className="py-2.5 px-3 text-center">-</td>
                <td className="py-2.5 px-3 text-right">-</td>
            ...
            <tr>
              ...
              <td className="py-2.5 px-3 text-right font-bold">{formatMoney((ps.roomCharge || 0) + (ps.extraBedCharge || 0) + (ps.extraPersonCharge || 0) + (ps.overtimeCharge || 0))}</td>
            </tr>
